...
function countPhotos(road) {
	let photoCount = 0;
	let leftCameraCount = 0;
	let rightCameraCount = 0;

	for (let i = 0, j = road.length - 1; i < road.length; i++, j--) {
		if (road[i] === '.') leftCameraCount++;
		if (road[j] === '.') rightCameraCount++;
		if (road[i] === '<') photoCount += leftCameraCount;
		if (road[j] === '>') photoCount += rightCameraCount;
	}
