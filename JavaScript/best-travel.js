...
function calculateDistances(distance, townsCount, distancesArr, combs = []) {
	if (townsCount === 0) {
		combs.push(distance);
		return combs;
	}
	...
	for (let i = 0; i < distancesArr.length; i++) {
		calculateDistances(distance + distancesArr[i], townsCount - 1, distancesArr.slice(i + 1), combs);
	}

	return combs;
}

function chooseBestSum(km, townsCount, array) {
	const combs = calculateDistances(0, townsCount, array);
	return combs.filter(el => el <= km).sort((a, b) => b - a)[0] || null;
}
